...
func TestServiceWorkerInstallUpdateAndOfflineNavigation(t *testing.T) {
	...
	if len(initialCaches) != 1 || !strings.HasPrefix(initialCaches[0], "pokget-shell-v") {
		t.Fatalf("initial cache names = %v, want one versioned pokget shell cache", initialCaches)
	}
	initialCache := initialCaches[0]
	updatedCache := strings.Replace(initialCache, "shell-v", "shell-test-v", 1)

	workerVersion.Store(1)
	if err := chromedp.Run(ctx,
		chromedp.Evaluate(`navigator.serviceWorker.getRegistration().then((registration) => registration.update())`, nil),
		chromedp.Poll(fmt.Sprintf(`caches.keys().then((names) => names.includes(%q) && !names.includes(%q))`, updatedCache, initialCache), nil,
			chromedp.WithPollingTimeout(20*time.Second)),
	); err != nil {
...
func newServiceWorkerTestServer(t *testing.T) (*httptest.Server, *atomic.Int32, *atomic.Bool) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "/sw.js":
			...
			if version.Load() > 0 {
				source = strings.Replace(source, "shell-v", "shell-test-v", 1)
			}
