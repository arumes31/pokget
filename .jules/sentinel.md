...
**Vulnerability:** The `Login` handler returned early when a user was not found in the database. This allowed attackers to enumerate existing users by measuring the response time, as successful queries would then spend time hashing the password.
**Learning:** To prevent timing attacks for user enumeration, avoid early returns on database misses.
**Prevention:** Use the application's actual hashing function (e.g., `_, _ = auth.HashPassword(password)`) when a user is not found to ensure the timing perfectly matches the current configuration.
