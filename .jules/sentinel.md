...
## 2025-02-14 - Timing Attack Prevention
**Vulnerability:** User enumeration via timing attack in Login handler.
**Learning:** Returning immediately on database misses (sql.ErrNoRows) before executing password hashing allows attackers to enumerate valid emails based on response times.
**Prevention:** Always execute the password hashing function against a constant dummy hash (e.g., a structurally valid bcrypt hash with the appropriate cost factor) to ensure constant response times.
