...
**Vulnerability:** The login endpoint returned early when a user was not found, taking significantly less time than when a user was found (due to skipping the expensive bcrypt password check).
**Learning:** This timing difference allows attackers to enumerate registered email addresses by measuring response times.
**Prevention:** Always perform a constant-time operation (like checking against a dummy bcrypt hash with the same cost factor) when a user is not found to equalize response times.
