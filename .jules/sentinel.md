...
## 2024-06-09 - Prevent User Enumeration Timing Attack
**Vulnerability:** The `Login` handler returned early when a user was not found (`sql.ErrNoRows`), taking significantly less time than a failed login for an existing user (which computes a password hash).
**Learning:** Early returns on database misses in authentication flows can allow attackers to enumerate valid usernames based on response times.
**Prevention:** Always perform the same computationally expensive operations (like password hashing) regardless of whether the user exists. Use the application's actual hashing function to ensure the timing matches exactly.
