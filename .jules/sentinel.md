...
## 2024-07-20 - Prevent User Enumeration via Timing Attack in Login
**Vulnerability:** The login endpoint was returning early on a database miss (`sql.ErrNoRows`) before calling `CheckPasswordHash`. This allowed an attacker to determine if an email existed by measuring response times.
**Learning:** Bcrypt hashing is deliberately slow. Bypassing it for non-existent users creates a measurable timing difference.
**Prevention:** Always execute the password hashing function against a constant dummy hash if the user is not found to ensure constant response times. The dummy hash must be a structurally valid string with the appropriate cost factor.
