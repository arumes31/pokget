...
## 2026-07-08 - Fix IP Spoofing in Rate Limiter
**Vulnerability:** The `RateLimitMiddleware` was applied before `ProxyMiddleware` in the HTTP router. This caused the rate limiter to evaluate the reverse proxy's IP address rather than the real client IP, allowing attackers to spoof their IP via headers and bypass rate limits.
**Learning:** The order of middleware execution is critical for security. Context-enriching middleware (like extracting real IPs) must always precede security mechanisms that rely on that context.
**Prevention:** Carefully review middleware chains to ensure data flow dependencies (e.g., IP extraction -> Rate Limiting) are respected in the setup order.
