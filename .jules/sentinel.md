...
## 2024-06-09 - Fix User Enumeration Timing Attack
**Vulnerability:** The Login handler returned early on a database miss, allowing attackers to enumerate users by observing response times.
**Learning:** Early returns in authentication flows skip expensive operations (like password hashing) when a user doesn't exist, which creates a noticeable timing difference.
**Prevention:** Always perform a dummy hash comparison using a structurally valid hash string to ensure constant response times on database misses.
