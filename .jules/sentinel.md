...
## 2024-06-08 - Fix User Enumeration Timing Attack in Login
**Vulnerability:** The Login handler returned a 401 Unauthorized immediately upon a database miss (`sql.ErrNoRows`), but proceeded to perform expensive password hashing (bcrypt) if the user existed.
**Learning:** This time difference allows an attacker to enumerate valid email addresses based on response times. To prevent timing attacks in authentication flows, the time taken for failed and successful attempts must be indistinguishable.
**Prevention:** Instead of returning early on database misses, always execute the expensive hashing function (e.g., `_, _ = auth.HashPassword(password)`) to ensure the response timing matches regardless of whether the user exists.
