...
## 2024-08-02 - Prevent User Enumeration Timing Attack in Login
**Vulnerability:** The Login handler returned early when a user was not found (`sql.ErrNoRows`), allowing attackers to enumerate registered emails via timing analysis (since verifying a password hash takes significantly longer than a DB miss).
**Learning:** Returning early on authentication misses creates a measurable timing discrepancy.
**Prevention:** Always perform the expensive cryptographic operation (hashing the provided password) even if the user is not found to normalize response times and prevent user enumeration.
