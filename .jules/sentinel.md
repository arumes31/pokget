...
## 2026-07-26 - Prevent User Enumeration via Timing Attacks
**Vulnerability:** The Login endpoint returned early when a user wasn't found in the database, skipping the expensive password hashing step and allowing attackers to enumerate valid emails via response times.
**Learning:** Returning early on database misses (e.g., `sql.ErrNoRows`) before performing slow cryptographic operations creates timing discrepancies.
**Prevention:** Always perform the expensive cryptographic operation (like `auth.HashPassword(password)`) even if the user lookup fails, ensuring constant response times.
