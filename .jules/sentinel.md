...
## 2026-07-23 - Prevent user enumeration timing attacks in Login handler
**Vulnerability:** User enumeration timing attack where early return on database miss (`sql.ErrNoRows`) allows attackers to identify registered emails.
**Learning:** To ensure constant response times, we must execute the password hashing function (bcrypt) even if the user isn't found. The dummy hash must be structurally valid with the correct cost factor (14).
**Prevention:** Always use a valid dummy hash for constant-time comparisons in authentication flows to mitigate timing attacks.
