...
## 2026-06-09 - JSON Decoding Overhead for HTTP Responses
**Learning:** Using `io.ReadAll` followed by `json.Unmarshal` allocates an unnecessary large byte slice for HTTP response bodies.
**Action:** Always prefer `json.NewDecoder(resp.Body).Decode(&dest)` combined with explicit body draining (`_, _ = io.Copy(io.Discard, resp.Body)`) to decode JSON from HTTP responses, avoiding large memory allocations and allowing HTTP connection reuse.
