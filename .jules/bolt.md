...
**Learning:** `io.ReadAll` combined with `json.Unmarshal` unnecessarily allocates a large byte slice for the entire HTTP response body in memory, which stresses the GC during high-volume API interactions (like LLM API calls).
**Action:** Prefer `json.NewDecoder(resp.Body).Decode(&dest)` to parse directly from the stream. Always remember to drain the remaining body (`io.Copy(io.Discard, resp.Body)`) to ensure the HTTP transport can correctly reuse the TCP connection (Keep-Alive).
